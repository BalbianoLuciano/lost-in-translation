...
![tests](https://img.shields.io/badge/tests-663%20Go%20·%2060%20Solidity%20·%2046%20web%20·%2029%20content%20·%208%20e2e-16a34a)
...
`pilar.ts` draws the pillar in normalised coordinates with Bézier curves;
Solidity has no floating point, so the same geometry lives in the contract in
integer thousandths. A piece token draws its piece with its two joints; an obra
token draws the whole pillar, assembled.
...
- **46 web tests**, one of which reads the real CSS tokens and fails if any text
  drops below its contrast floor.
- **29 content tests** for the validator's rules, including one that checks the
